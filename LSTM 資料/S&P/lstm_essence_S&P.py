...
class Model_patten:
  def __init__(self, X_train, y_train, X_val, y_val):
        ...
        self.model.add(LSTM(units=64, dropout=0.2, return_sequences=True))
        self.model.add(LSTM(units=64, dropout=0.2, return_sequences=False))
  ...
  def get_loss(self):
    ...
    plt.savefig('./S&P_Result/S&P_Loss.png')
    plt.show()
  ...
  def get_acc(self):
    ...
    plt.xlabel('Epoch')
    plt.legend(['Train', 'Validation'], loc='upper left')
    plt.savefig('./S&P_Result/S&P_Accuracy.png')
    plt.show()
    plt.close()
...
class PlotGraph:
    ...
    def predict_graph(self, result, actual, timestep):
        ...
        # plt.ylim([-12, 12])
        plt.legend()
        plt.savefig('./S&P_Result/S&P_PCT_Prediction.png')
        plt.show()        
        plt.close()
...
plt.legend()
plt.savefig('./S&P_Result/S&P_PCT_Histogram.png')
plt.show()
# plt.pause(5)
plt.close()

print(train_feature['feature'].value_counts())
...
pred_7 = list()
test_d7 = test_data_7.iloc[6,:]
test_d7C = test_d7['Close']

for i in i7:
  test_d7C = test_d7C * i + test_d7C
...
pg.predict_graph(i7, Actual_feature['feature_check'], 7)
