...
class Model_patten:
  ...
  def get_loss(self):
    ...
    plt.savefig('./NASDAQ_Result/NASDAQ_Loss.png')
    plt.show()
    plt.close()
  ...
  def get_acc(self):
    ...
    plt.xlabel('Epoch')
    plt.legend(['Train', 'Validation'], loc='upper left')
    plt.savefig('./NASDAQ_Result/NASDAQ_Accuracy.png')
    plt.show()
...
class PlotGraph:
    ...
    def predict_graph(self, result, actual, timestep):
        ...
        # plt.ylim([-12, 12])
        plt.legend()
        plt.savefig('./NASDAQ_Result/NASDAQ_PCT_Prediction.png')
        plt.show()
...
raw_data = pd.read_csv("./NASDAQ.csv")
...
plt.legend()
plt.savefig('./NASDAQ_Result/NASDAQ_PCT_Histogram.png')
plt.show()
plt.close()
